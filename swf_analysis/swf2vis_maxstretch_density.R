...
dfmax_stretch<-function(df)
    return(max(df$stretch))

m=0
for (i in 1:2){
    df=data_ready_to_plot[[i]]
    m=max(c(dfmax_stretch(df),m))
}
print(m)
verb(data_ready_to_plot,d="Data ready to plot:")

plot_user_stretch_density(data_ready_to_plot,upperbound=m,xlabel="Max Stretch",ylabel="Density",bw=args$grey,labelnames=args$labels,labelstitle=args$labelstitle)
