...
description='This tool will plot all three graphs for visualizing small swf logs from a campaign scheduling algorithm. (usually a simulator). It does the \'glesser\' plot, the ostrich virtual schedule, and the gantt chart of the system.'
epilog='Use as much swf files as you wish. The data will be cat into a pdf.'
...
userfiles=c('analysis.R','visu.R','ostrich.R')
...
#type stuff here.
allostrich_one_file <- function( swf_filename ) {
	data=swf_read(swf_filename)
	gp1=df2glesserchart(data)
	pause_output(options_vector)
	gp2=df2gantt(data)
	pause_output(options_vector)
	df2virtual_schedule(data)
}
trace(allostrich_one_file)
lapply(args$swf_filename,allostrich_one_file)
